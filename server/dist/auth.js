...
export function signToken(user) {
    return jwt.sign({ sub: user.id, email: user.email, role: user.role, name: user.name }, SECRET, { expiresIn: ACCESS_TTL });
}
...
export function authenticate(req, _res, next) {
    ...
    else if (req.cookies && req.cookies.maison_token)
        token = req.cookies.maison_token;
    if (token) {
        try {
