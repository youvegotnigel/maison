...
function getCartId(buyerId) {
    const cart = db.prepare('SELECT id FROM carts WHERE buyer_id = ?').get(buyerId);
    if (!cart) {
...
function buildCart(buyerId) {
    ...
    const items = db.prepare('SELECT ci.id AS itemId, ci.quantity, p.id AS productId FROM cart_items ci JOIN products p ON p.id = ci.product_id WHERE ci.cart_id = ?').all(cartId);
    let subtotal = 0;
...
router.delete('/items/:itemId', (req, res) => {
    ...
    if (!item)
        return fail(res, 404, 'ITEM_NOT_FOUND', 'That cart item does not exist.');
...
ordersRouter.get('/:reference', (req, res) => {
    ...
    if (!o)
        return fail(res, 404, 'ORDER_NOT_FOUND', 'That order does not exist.');
    res.json({ order: {
...
ordersRouter.post('/', (req, res) => {
    ...
    try {
        db.exec('BEGIN');
        const orderId = db.prepare('INSERT INTO orders (buyer_id, reference, status, total_cents, shipping_json) VALUES (?, ?, ?, ?, ?)').run(req.user.sub, reference, 'confirmed', cart.subtotalCents, JSON.stringify(shipping)).lastInsertRowid;
        const insItem = db.prepare('INSERT INTO order_items (order_id, product_id, name, quantity, unit_price_cents) VALUES (?, ?, ?, ?, ?)');
