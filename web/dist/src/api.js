...
async function request(method, path, body) {
    ...
    if (!res.ok) {
        const j = json;
        throw new ApiError(j?.error?.message || res.statusText, j?.error?.code || 'HTTP_' + res.status, res.status, json);
    }
    return json;
}
export const api = {
    ...
    certificate: (id) => request('GET', `/products/${id}/certificate`),
    issueCertificate: (id) => request('POST', `/products/${id}/certificate`),
